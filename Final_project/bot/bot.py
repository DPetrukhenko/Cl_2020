...
def recommend_free(message):
    ...
    if len(games) == 0:
      ...
      'Кажется, у тебя приватный аккаунт. Проверь, видны ли другим людям твои игры и попробуй снова.'
      )
    else: 
        loc_data = data
        clusters_use = []
...
def recommend_free(message):
    ...
    games = get_games(user_url)
    if len(games) == 0:
      bot.send_message(message.from_user.id, 'Кажется, у тебя приватный аккаунт. Проверь, видны ли другим людям твои игры и попробуй снова.')
    loc_data = data
    clusters_use = []
    clusters_tf_idf = []
    clusters_bert = []
